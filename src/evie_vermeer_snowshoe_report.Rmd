...
```{r, include = FALSE}
juvenile_hare_lm <- lm(weight ~ hindft, data=juvenile_hares)
...
```
Upon running a linear regression of these data, we see a regression slope of ~9.5, meaning that the average juvenile snowshoe hare's weight increases 9.5 grams for every mm of hindfoot length increase. However, the R-squared value of this regression is low (R = .3), meaning that value of sample weight can not be well-predicted by hindfoot length in this model. Additionally, a linear regression assumes homoskedasticity of data, and the plot above lends concern to this assumption as the data as it appears heteroskedastic. 
